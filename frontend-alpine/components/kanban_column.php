<div class="flex flex-col flex-1 min-w-[300px] bg-slate-800/40 rounded-2xl border border-slate-700/60 overflow-hidden shadow-lg">
    <div class="bg-slate-800/90 px-4 py-3 border-b border-slate-700 flex justify-between items-center backdrop-blur-sm z-10">
        ...
        <span class="bg-slate-700/50 border border-slate-600 text-slate-200 text-xs font-black px-2.5 py-1 rounded-full"
              x-text="$store.kds.groupedColumns[type].length"></span>
    </div>
    <div class="p-3 overflow-y-auto flex-1 custom-scrollbar">
        <template x-if="$store.kds.groupedColumns[type].length === 0">
            <div class="h-full flex flex-col items-center justify-center text-slate-500 opacity-60">
                ...
                <p class="font-semibold text-sm">Estación Libre</p>
            </div>
        </template>
        
        <template x-for="item in $store.kds.groupedColumns[type]" :key="item.ids[0]">
            <?php include 'kanban_card.php'; ?>
        </template>
    </div>
</div>
